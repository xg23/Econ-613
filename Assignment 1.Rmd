...
id_ss <- stu %>%
  select(X, schoolcode1:schoolcode6, jssdistrict, rankplace) %>%
  gather(key = 'seniorhigh', 'schoolcode', -X, -jssdistrict, -rankplace) %>%
...
stu6 <- left_join(stu4, admin, by = "choice_sp") %>% 
  select(choice_sp, schoolcode, sssdistrict:size, rankplace) %>%
