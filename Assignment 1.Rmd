...
df_schpgm <- left_join(school, pgm, by = c("X", "choice")) %>%
   mutate(choice_sp = paste(schoolcode, choicepgm, sep = ",")) # id var is choice of school_pgm

# Create a school_individual level dataframe: df_ss
```
...
*stu2* presents the number of students apply to the same school.
```{r}
stu2 <- school %>%
  drop_na(schoolcode) %>%
  group_by(schoolcode) %>%
...
*avg_sd* is the table for average and sd of cutoff, quality and distance by ranked choice. *avg_sd_quantile* 
```{r}
# get cutoff and quality from exercise 2
stu6 <- stu %>%
  
  left_join(stu4, admin, by = "choice_sp") %>% 
  select(choice_sp, schoolcode, sssdistrict:size, rankplace, score) %>%
  filter(!is.na(rankplace)) %>% # add rankplace as the id variable to stu5 in exercise 2
  filter(rankplace < 7) %>% 
  distinct() 
...
quality_q <- stu7 %>%
  ...
  group_by(quantile, rankplace) %>%
  summarise(mean_cutoff = mean(quality), sd_cutoff = sd(quality))
...
avg_sd_quantile <- left_join(cutoff_q, quality_q, dist_q, by = c("quantile", "rankplace")) %>%
...
```{r}
 stu8 <- stu %>%
  ...
  mutate(choice = str_match(school, "[1-6]") %>% as.numeric()) %>%
  filter(!is.na(rankplace) & !is.na(score)) %>% 
  filter(rankplace < 7) 
